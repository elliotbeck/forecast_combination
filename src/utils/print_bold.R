...
colnames_sanitizer <- function(x) {
  paste0(paste("\\textbf{", x, "}"))
}

printbold <- function(x, which = NULL, each = c("column", "row"), max = TRUE,
                      NA.string = "", type = c("latex", "html"), digits_round = 2,
                      sanitize.text.function = force,
                      sanitize.rownames.function = NULL,
                      sanitize.colnames.function = colnames_sanitizer, ...) {
  stopifnot(inherits(x, "xtable"))
  each <- match.arg(each)
  type <- match.arg(type)
  digits <- rep(xtable::digits(x), length = ncol(x) + 1)
  if (!is.null(which)) {
  ...
  display <- rep(xtable::display(x), length = ncol(x) + 1)
  for (i in 1:ncol(x)) {
    if (!is.numeric(x[, i])) next
    ina <- is.na(x[, i])
    x[, i] <- formatC(x[, i],
